...
function runP4AndMergeResult(processArgs, stdInput, result) {
    ...
    try {
        ...
        if(err.errorCode === studio.system.processError.Timedout) {
            err.errorString = "The Perforce operation has timed out due to extended period of inactivity. Please make sure the server is online and accessible via your network.";
        }
        throw err;
    }
...
function runP4Impl(command, additionalArgs, stdInput) {
    var base = "-z tag -I"; // output result of command in fstat format and show progress indicator when possible
...
function tryRunP4(command, callback) {
    var standardError = undefined;
    try {
        var result = runP4(command);
...
function verifyWorkspaceSelection(widget) {
    ...
    if(projectRoot && projectRoot.length) {
        ...
        tryRunP4("where " + projectRoot, function(result) { });
    }
